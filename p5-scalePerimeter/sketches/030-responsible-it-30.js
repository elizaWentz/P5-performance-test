...
function buildTrianglePlate() {
  let plateCenterX = 566;
  let plateCenterY = 152.6;
  let horizontalSpread = 0.78;
  let verticalSpread = 2.25;
  let canvasCenterX = width * 0.5;
  let canvasCenterY = height * 0.5;
  ...
  for (let i = 0; i < centers.length; i++) {
    let c = centers[i];
    triangles.push({
      x: (c[0] - plateCenterX) * horizontalSpread + canvasCenterX,
      y: (c[1] - plateCenterY) * verticalSpread + canvasCenterY,
      r: c[2],
      flip: c[3],
...
function drawRisoMixerTriangles() {
  ...
  let cols = 10;
  let rows = 6;
  let startX = width * 0.5 - cols * cellW * 0.5;
  let startY = height * 0.5 - rows * cellH * 0.5;
  ...
  for (let col = 0; col < cols; col++) {
    for (let row = 0; row < rows; row++) {
      ...
      let alphaValue = map(col + row, 0, cols + rows - 2, 38, 104);
...
function drawTriangleMesh() {
  ...
  for (let x = -36; x < width + 112; x += meshStepX) {
    for (let y = 24; y < height + 96; y += meshStepY) {
      let flip = (floor(x / meshStepX) + floor(y / meshStepY)) % 2 === 0 ? 1 : -1;
      let r = 46 + noise(x * 0.01, y * 0.01) * 34;
      let p = trianglePoints(x, y, r, flip, 0);
      let c = (floor(x / meshStepX) + floor(y / meshStepY)) % 3 === 0 ? inks.seaBlue : inks.fluorescentPink;

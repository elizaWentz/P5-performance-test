...
function draw() {
  ...
  // Basis-kleuren: donker olijfgroen boven, lichter onder
  let topCol    = color(85, 107, 47); // dark olive green
  let middleCol = color(107, 142, 35); // lighter olive green
  let bottomCol = color(143, 188, 143); // pale green
  ...
  for (let x = 0; x < width; x += warpStep) {
    let t = x / width;
    let warpCol = lerpColor(color(200), color(85, 107, 47), 0.7);
  ...
  for (let y = 0; y < height; y += weftStep) {
    let ny = y / height;
    let weftCol = lerpColor(color(255), color(85, 107, 47), 0.6);

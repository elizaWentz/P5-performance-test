...
function setup() {
  ...
  // New palette based on image 2 (brown / olive / cream)
  jungleBlack = color('#181A14'); // deepest near-black olive
  jungleDark  = color('#5A3A23'); // dark brown
  deepGreen   = color('#8B5A2B'); // warm brown
  mossGreen   = color('#6C6B3B'); // olive green
  leafGreen   = color('#A08355'); // tan / mid highlight
  sunGreen    = color('#E7E0D0'); // light cream highlight
  hazeGreen   = color('#B7AD90'); // soft, dusty light tone
  goldMist    = color('#D0C29A'); // warm golden mist
...
class RainforestTriangle {
  ...
  draw(t) {
    ...
    let light = lerpColor(leafGreen, sunGreen,
      smooth01(1 - corridor) * 0.5 + smooth01(1 - vertical) * 0.32
    );

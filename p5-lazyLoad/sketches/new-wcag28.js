...
function draw() {
  ...
  // Basis-kleuren: donker rood boven, lichter onder
  let topCol    = color(128, 0, 0); // burgundy
  let middleCol = color(165, 42, 42); // brownish red
  let bottomCol = color(220, 20, 60); // crimson
  ...
  for (let x = 0; x < width; x += warpStep) {
    let t = x / width;
    let warpCol = lerpColor(color(200), color(128, 0, 0), 0.7);
  ...
  for (let y = 0; y < height; y += weftStep) {
    let ny = y / height;
    let weftCol = lerpColor(color(255), color(128, 0, 0), 0.6);
